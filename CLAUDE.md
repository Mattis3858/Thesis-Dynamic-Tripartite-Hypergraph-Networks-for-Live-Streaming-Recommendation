...
```
`run_experiments.ps1` is the thesis ablation/hyperparameter sweep batch (comments are in Traditional Chinese). Thesis tables/figures are produced by `eval_table10.py` (per-user-bias-group metrics), `eval_table13.py` (tripartite vs. masked-2D ablation), `eval_coldstart.py` (zero-history scenarios), and `analyze_user_bias.py` (KMeans user grouping → `experiment_results/user_bias_groups.csv` + plots). These eval scripts load saved checkpoints and import helpers from each other (`eval_table13`/`eval_coldstart` import from `eval_table10`).
...
- **Training**: BCE (default) or BPR loss with `--train_neg_ratio` random negatives `(v', w')` per positive. Per-epoch validation is a fast 1-negative BCE on `val_data`; early stopping minimizes val loss (`utils/EarlyStopping.py`). `--early_stop_metric` can instead monitor a ranking metric (`val_ndcg@5`, …) — wired but off by default (using it changes model selection and means retraining).
